...
use Modules\School\Interfaces\UserInterface;
use Modules\School\Traits\Base64ToFile;
...
class UserRepository implements UserInterface {
    ...
    public function updateStudent($request) {
        try {
            DB::transaction(function () use ($request) {
                User::query()->where('id', $request['student_id'])->update([
                    'first_name'    => $request['first_name'],
                    'last_name'     => $request['last_name'],
                    'email'         => $request['email'],
                    'gender'        => $request['gender'],
    ...
    public function updateTeacher($request) {
        try {
            DB::transaction(function () use ($request) {
                User::query()->where('id', $request['teacher_id'])->update([
                    'first_name'    => $request['first_name'],
                    'last_name'     => $request['last_name'],
                    'email'         => $request['email'],
                    'gender'        => $request['gender'],
    ...
    public function getAllStudents($session_id, $class_id, $section_id) {
        if($class_id == 0 || $section_id == 0) {
            $schoolClass = SchoolClass::query()->where('session_id', $session_id)
                ->first();
            $section = Section::query()->where('session_id', $session_id)
                ->first();
    ...
    public function findStudent($id) {
        try {
            return User::query()->with('parent_info', 'academic_info')->where('id', $id)->first();
        } catch (\Exception $e) {
    ...
    public function findTeacher($id) {
        try {
            return User::query()->where('id', $id)->where('role', 'teacher')->first();
        } catch (\Exception $e) {
    ...
    public function getAllTeachers() {
        try {
            return User::query()->where('role', 'teacher')->get();
